'use client'
import { useEffect, useState } from 'react'
import { useRouter } from 'next/navigation'

export default function FormatoPublicoClient({ publicLink }) {
  const router = useRouter()
  const [mostrarDialogo, setMostrarDialogo] = useState(false)
  const [cargando, setCargando] = useState(true)

  useEffect(() => {
    const verificarAcceso = async () => {
      try {
        const res = await fetch(`/api/formato/publico/${publicLink}`, {
          credentials: 'include',
        })

        const data = await res.json()

        if (res.status === 200 && data.acceso === true) {
          router.push(`/dashboard/formato/${data.formatoId}`)
        } else {
          setMostrarDialogo(true)
        }
      } catch (err) {
        console.error('❌ Error verificando acceso:', err)
        setMostrarDialogo(true)
      } finally {
        setCargando(false)
      }
    }

    verificarAcceso()
  }, [publicLink, router])

  const aceptarFormato = async () => {
    try {
      const res = await fetch('/api/formato/aceptar', {
        method: 'POST',
        body: JSON.stringify({ publicLink }),
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      })

      if (res.ok) {
        const data = await res.json()
        router.push('/') 
      } else {
        const error = await res.json()
        alert(error?.error || 'No se pudo aceptar el formato.')
      }
    } catch (err) {
      console.error('❌ Error al aceptar formato:', err)
      alert('Ocurrió un error al aceptar el formato.')
    }
  }

  if (cargando) return <p className="p-6">Cargando...</p>

  if (mostrarDialogo) {
    return (
      <div className="p-6">
        <p>¿Deseas recibir este formato en tu dashboard?</p>
        <button
          onClick={aceptarFormato}
          className="bg-blue-500 text-white p-2 rounded mt-4 hover:bg-blue-600 transition"
        >
          Sí, aceptar
        </button>
      </div>
    )
  }

  return null
}
